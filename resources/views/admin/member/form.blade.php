...
@section('contents')
    ...
    <div class="flex flex-row">
        @if($title == 'Add')
            ...
            <a href="{{ route('member_listing') }}" class=" font-semibold text-md text-blue-500 rounded-lg px-5 py-2.5 text-center inline-flex items-center dark:focus:ring-gray-500 dark:hover:bg-[#050708]/30 mr-2 mb-2">
                <svg class="h-4 w-4 text-white"  fill="none" viewBox="0 0 24 24" stroke="#3F83F8">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16l-4-4m0 0l4-4m-4 4h18"/>
                </svg>
                <span class="ml-4 text-xl text-blue-400">@lang('public.cancel')</span>
            </a>
        @elseif($title == 'Edit')
            <h1 class="flex-1 font-semibold text-2xl text-blue-500">@lang('public.members')  / {{ @$user->name }} - @lang('public.member_edit')</h1>
            <a href="{{ route('member_details', @$user->id) }}" class=" font-semibold text-md text-blue-500 rounded-lg px-5 py-2.5 text-center inline-flex items-center dark:focus:ring-gray-500 dark:hover:bg-[#050708]/30 mr-2 mb-2">
                <svg class="h-4 w-4 text-white"  fill="none" viewBox="0 0 24 24" stroke="#3F83F8">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16l-4-4m0 0l4-4m-4 4h18"/>
                </svg>
                <span class="ml-4 text-xl text-blue-400">@lang('public.cancel')</span>
    ...
    <form method="post" action="{{ $submit }}" enctype="multipart/form-data">
        @csrf
        <div class="bg-blue-100 shadow-lg p-8">
            <h3 class="text-blue-500 font-bold text-lg mb-4">@lang('public.member_access')</h3>
            <div class="grid gap-6 mb-6 md:grid-cols-2">
                <div>
                    <label for="email" class="block mb-2 font-semibold text-sm text-blue-400 dark:text-white">@lang('public.email_address')</label>
                ...
                <div>
                    <label for="rank" class="block mb-2 font-semibold text-sm text-blue-400 dark:text-white">@lang('public.role')</label>
                    {!! Form::select('role', $get_role_sel, @$post->role, ['class' => 'font-medium text-sm text-gray-500 bg-gray-50 border border-gray-300 text-sm placeholder:text-gray-400 text-gray-500 rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500']) !!}
                </div>
                <div>
                    <label for="password" class="flex block mb-2 text-sm font-semibold text-blue-400">
                    ...
                    <input type="password" id="password" name="password" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 placeholder:text-gray-400 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 @error('password') bg-red-50 border border-red-500 text-red-900 placeholder-red-700 text-sm rounded-lg focus:ring-red-500 focus:border-red-500 block w-full p-2.5 dark:bg-red-100 dark:border-red-400 @enderror" placeholder="@lang('public.password')" value="{{ @$post->password }}">
                    @error('password')
                    <div class="mt-2 text-sm text-red-600">{{ $message }}</div>
                    @enderror
                </div>
                <div>
                    <label for="confirm_password" class="block mb-2 font-semibold text-sm text-blue-400 dark:text-whit">@lang('public.password_confirmation')</label>
                    <input type="password" id="confirm_password" name="password_confirmation" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 placeholder:text-gray-400 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 @error('password') bg-red-50 border border-red-500 text-red-900 placeholder-red-700 text-sm rounded-lg focus:ring-red-500 focus:border-red-500 block w-full p-2.5 dark:bg-red-100 dark:border-red-400 @enderror" placeholder="@lang('public.password_confirmation')" value="{{ @$post->confirm_password ?? @$post->password }}">
                </div>
            </div>
        </div>
        <div class="bg-blue-100 shadow-lg p-8 mt-6">
            <h3 class="text-blue-500 font-bold text-lg mb-4">@lang('public.member_details')</h3>
            @if($title == 'Edit')
                <div class="flex flex-col items-center p-8">
                    <div class="relative inline-flex items-center justify-center w-20 h-20 overflow-hidden bg-blue-400 rounded-full dark:bg-gray-600">
                        @if (@$user->profile_image)
                            <img src="{{ asset('uploads/users/' .@$user->profile_image)}}" id="profile_pic_preview" class="relative inline-flex items-center justify-center w-32 h-32 overflow-hidden bg-blue-400 rounded-full dark:bg-gray-600font-bold text-white dark:text-gray-300 text-4xl object-contain">
                        @else
            ...
            <div class="grid gap-6 mb-6 md:grid-cols-2">
                <div>
                    <label for="name" class="block mb-2 font-semibold text-sm text-blue-400 dark:text-white">@lang('public.name')</label>
                ...
                <div>
                    <label for="address" class="block mb-2 font-semibold text-sm text-blue-400 dark:text-white">@lang('public.address')</label>
                ...
                <div>
                    <label for="rank" class="block mb-2 font-semibold text-sm text-blue-400 dark:text-white">@lang('public.rank')</label>
                ...
                <div>
                    <label for="contact_number" class="block mb-2 font-semibold text-sm text-blue-400 dark:text-white">@lang('public.contact')</label>
                ...
                <div>
                    <label for="countries" class="block mb-2 text-sm font-semibold text-blue-400 dark:text-white">@lang('public.country')</label>
                    {!! Form::select('country', $get_country_sel, @$post->country, ['class' => 'font-medium text-sm placeholder:text-gray-400 text-gray-500 bg-gray-50 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500']) !!}
                </div>
                <div>
                    <label for="status" class="block mb-2 font-semibold text-sm text-blue-400 dark:text-white">@lang('public.status')</label>
                    {!! Form::select('status', $get_status_sel, @$post->status, ['class' => 'font-medium text-sm placeholder:text-gray-400 text-gray-500 bg-gray-50 border border-gray-300 text-sm text-gray-500 rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500']) !!}
                </div>
                <div>
                    <label for="leader_status" class="block mb-2 font-semibold text-sm text-blue-400 dark:text-white">@lang('public.leader_status')</label>
