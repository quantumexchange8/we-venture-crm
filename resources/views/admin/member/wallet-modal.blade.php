<!-- Approval Modal -->
<div id="wallet_approval_modal-{{ $record->id }}" tabindex="-1" aria-hidden="true" class="fixed top-0 left-0 right-0 z-50 hidden w-full p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] md:h-full">
    <div class="relative w-full h-full max-w-md md:h-auto">
        <!-- Modal content -->
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
            <button type="button" class="absolute top-3 right-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center dark:hover:bg-gray-800 dark:hover:text-white" data-modal-hide="wallet_approval_modal-{{ $record->id }}">
                <svg aria-hidden="true" class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd"></path></svg>
                <span class="sr-only">@lang('public.close_modal')</span>
            </button>
            <div class="px-6 py-6 lg:px-8">
                <form method="post" action="{{ route('member_wallet_approval', $record->id) }}">
                    @csrf
                    <div class="flex justify-center">
                        <div class="flex flex-col h-full">
                            ...
                            <div class="flex-grow p-5">
                                ...
                                <!-- Description -->
                                <div class="mt-2">
                                    <p class="font-medium text-gray-500 dark:text-gray-400 ">@lang('public.date_submit'): <b>{{ date('Y-m-d h:i A', strtotime($record->requested_at)) }}</b></p>
                                </div>
                                <div class="mt-2">
                                    <p class="font-medium text-gray-500 dark:text-gray-400 ">@lang('public.wallet_type'): <b class="uppercase">{{ $record->wallet_type }}</b></p>
                                </div>
                                <div class="mt-2">
                                    <p class="font-medium text-gray-500 dark:text-gray-400 ">@lang('public.current_wallet_address'): <b>{{ $record->wallet_address }}</b></p>
                                </div>
                                <div class="mt-2">
                                    <p class="font-medium text-gray-500 dark:text-gray-400 ">@lang('public.new_wallet_address'): <b>{{ $record->wallet_address_request }}</b></p>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="text-center">
                        <button type="submit" class="text-white bg-[#40DD7F] hover:bg-green-400 focus:ring-4 focus:outline-none focus:ring-green-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center mr-2 dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-800" value="approve" name="wallet_address_request_status">
