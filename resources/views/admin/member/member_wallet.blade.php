...
@section('contents')
    ...
    <div class="flex flex-col">
        <form method="post" action="{{ route('member_wallet') }}">
            @csrf
            <div class="grid gap-6 mb-6 mt-4 grid-cols-2 max-[1300px]:grid-cols-1 max-[1000px]:grid-cols-1 max-[770px]:grid-cols-1">
                ...
                <div>
                    {!! Form::select('wallet_address_request_status', $get_status_sel, @$search['wallet_address_request_status'], ['class' => 'bg-gray-50 border border-gray-300 text-gray-500 text-sm rounded-lg focus:ring-orange-500 focus:border-orange-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-orange-500 dark:focus:border-orange-500']) !!}
                </div>
                <div>
        ...
        <div class="relative overflow-x-auto shadow-md sm:rounded-lg mt-2">
            <table class="w-full text-left">
                ...
                <tr>
                    ...
                    <th scope="col" class="p-4">
                        @lang('public.wallet_type')
                    </th>
                    <th scope="col" class="p-4">
                        @lang('public.wallet_address')
                    </th>
                    <th scope="col" class="p-4">
                        @lang('public.request_status')
                    </th>
                    <th scope="col" class="p-4">
                        @lang('public.wallet_status')
                    </th>
                    <th scope="col" class="p-4">
                ...
                @foreach($records as $record)
                    <tr class="border-b odd:bg-[#F6F6F6] even:bg-[#FDFCF3] text-sm">
                        ...
                        <td class="p-4">
                            @if($record->wallet_address_request_status === \App\Models\UserWallet::STATUS_PENDING)
                                <a href="#wallet_approval_modal-{{ $record->id }}" class="underline text-[#1A8BFF]"
                                   data-modal-target="wallet_approval_modal-{{ $record->id }}" data-modal-toggle="wallet_approval_modal-{{ $record->id }}"
                                   data-te-ripple-init
                            ...
                            @else
                                {{ $record->user->name }}
                            @endif
                            @include('admin.member.wallet-modal')
                        </td>
                        <td class="p-4">
                            {{ $record->user->email }}
                        </td>
                        ...
                        <td class="p-4">
                            ...
                            @elseif($record->wallet_address_request_status == \App\Models\UserWallet::STATUS_APPROVED)
                                <span class="text-success font-semibold uppercase">@lang('public.approved')</span>
                            @elseif($record->wallet_address_request_status == \App\Models\UserWallet::STATUS_REJECTED)
                                <span class="text-danger font-semibold uppercase">@lang('public.rejected')</span>
                            @endif
                        </td>
                        <td class="p-4">
                            @if($record->wallet_status == \App\Models\UserWallet::STATUS_ACTIVE)
                                <span class="text-success font-semibold uppercase">@lang('public.active')</span>
                            @elseif($record->wallet_status == \App\Models\UserWallet::STATUS_INACTIVE)
                                <span class="text-warning font-semibold uppercase">@lang('public.inactive')</span>
                            @endif
                        </td>
                        <td class="p-4">
                            {{ date('Y-m-d h:i A', strtotime($record->requested_at)) }}
                        </td>
                    </tr>
                        <?php
                        $no++;
