...
@section('contents')
    <h1 class="font-semibold text-2xl text-gray-500">@lang('public.portfolio')</h1>

    <!-- search -->
    <div class="flex flex-col">
        <form method="post" action="{{ route('portfolio_listing') }}">
            @csrf
            <div class="grid gap-6 mb-6 mt-4 md:grid-cols-1">
                ...
                <div class="relative">
                    ...
                    <input type="text" id="search" class="block w-full p-2.5 pl-10 text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-3F83F8-500 focus:border-3F83F8-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-3F83F8-500 dark:focus:border-blue-500" placeholder="@lang('public.search_name')" name="freetext" value="{{ @$search['freetext'] }}">
                </div>
                <div class="max-[755px]:flex max-[755px]:flex-col gap-2">
                    ...
                    <a href="{{ route('add_portfolio') }}" class="inline-flex text-white py-1.5 px-8 rounded bg-[#2AC769] hover:bg-success-500 text-lg font-bold float-right items-center justify-center">
                        <div class="flex items-center">
                            <svg class="h-6 w-6 text-white"  fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/>
                            </svg>
                            <span class="ml-4 text-md font-bold">@lang('public.add_portfolio')</span>
                        </div>
                    </a>
                </div>
            </div>
                    ...
                    <tr class="border-b odd:bg-[#FFFFFF] even:bg-blue-100">
                        ...
                        <td class="px-6 py-4">
                            @if($record->status == 'active')
                                <span class="font-semibold text-success">@lang('public.active')</span>
                            @else
                                <span class="font-semibold text-danger">@lang('public.inactive')</span>
                            @endif
                        </td>
                        <td class="px-6 py-4">
                        ...
                        <td class="px-6 py-4">
                            <div class="flex gap-2">
                                <a href="{{ route('portfolio_edit', $record->id) }}">
        ...
        <div class="relative w-full h-full max-w-md md:h-auto">
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                ...
                <div class="p-6 text-center">
                    <form method="POST" action="{{ route('portfolio_delete') }}">
                        @csrf
                        <svg aria-hidden="true" class="mx-auto mb-4 text-gray-400 w-14 h-14 dark:text-gray-200" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                        <div class="modal-body">
                            <h3 class="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">@lang('public.delete_portfolio_confirmation')</h3>
    ...
    <script>
        $(document).ready(function(e) {
            $('.delete').on('click', function() {
                var id = $(this).attr('id');
                $(".modal-body #portfolio_id").val( id );
            });
        });
    </script>
