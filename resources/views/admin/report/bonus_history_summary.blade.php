...
@section('contents')
    ...
    <div class="flex flex-col">
        <form method="post" action="{{ route('bonus_history_summary') }}">
            @csrf
            ...
            <table class="w-full text-left">
                ...
                @foreach($records as $record)
                    <tr class="border-b odd:bg-[#FFFFFF] even:bg-blue-100 text-sm">
                        ...
                        <td class="px-6 py-4">
                            {{ number_format($record->total_commission_lot,2) }}
                        </td>
                        <td class="px-6 py-4">
                            $ {{ number_format($record->total_bonus_amount,2) }}
                        </td>
                        <td class="px-6 py-4">
                            <span>{{ trans('public.' . $record->bonushistory_type) }}</span>
                        </td>
                    </tr>
                @endforeach
