...
@section('contents')
    <div class="flex flex-row">
        <h1 class="flex-1 font-semibold text-2xl text-gray-500">@lang('public.setting') / @lang('public.lot_size')</h1>
        <a href="{{ route('setting_listing') }}" class="text-xl font-semibold text-[#FFA168]">@lang('public.back')</a>
    </div>

    <form class="space-y-6 md:w-1/2 sm:w-full mb-6" action="{{ $submit }}" method="post">
        @csrf
        <div class="mb-6">
            <label for="lot_size" class="block mb-2 font-bold text-orange-400 dark:text-white">@lang('public.lot_size')</label>
            <input type="text" id="lot_size" name="lot_size" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-orange-400 focus:border-orange-400 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-orange-400 dark:focus:border-orange-400 @error('lot_size') bg-red-50 border border-red-500 text-red-900 placeholder-red-700 text-sm rounded-lg focus:ring-red-500 focus:border-red-500 block w-full p-2.5 dark:bg-red-100 dark:border-red-400 @enderror" placeholder="Lot Value">
            @error('lot_size')
    ...
    <h2 class="block mb-4 font-bold text-orange-400 dark:text-white">@lang('public.history')</h2>
