<div
    ...
    id="edit_plan-{{ $record->id }}"
    tabindex="-1"
    aria-labelledby="edit_plan-{{ $record->id }}_label"
    aria-hidden="true">
    <div
        ...
        <div
            ...
            <form action="{{ route('investment.edit_plan') }}" method="POST" enctype="multipart/form-data" id="edit_plan_{{ $record->id }}" class="p-6">
                @csrf
                <input type="hidden" name="investment_id" value="{{ $record->id }}">
                <input type="hidden" name="portfolio_id" value="{{ $record->portfolio->id }}">
                <div class="mb-4">
                    <label for="balance"
                           class="block mb-2 text-sm font-semibold text-gray-900 dark:text-white">@lang('public.min_deposit') (USD)
                    </label>
                    <input type="text" name="balance" id="balance" aria-label="disabled input 1" disabled
                           readonly value="{{number_format($record->portfolio->min_amount, 2)}}"
                ...
                <div class="mb-4">
                    ...
                    <input type="number" name="amount" id="deposit_amount"
                           ...
                           class="block w-full p-2.5 text-sm text-orange-400 border border-gray-300 rounded-lg bg-gray-50 focus:ring-orange-500 focus:border-orange-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-orange-500 dark:focus:border-orange-500 amount"
                    >
