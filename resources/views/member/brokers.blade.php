...
@section('contents')
        ...
        <nav class="flex " aria-label="Breadcrumb">
            <ol class="inline-flex items-center space-x-1 md:space-x-3 text-lg font-semibold">
                ...
                <div class="flex items-center">
                    ...
                    <p href="#" class="ml-1  text-gray-700 hover:text-blue-600 md:ml-2 dark:text-gray-400 dark:hover:text-white">@lang('public.broker_list')</p>
                </div>
                </li>
            </ol>
        </nav>
        <div class="grid grid-cols-3 gap-6 my-4 max-[1100px]:grid-rows-auto max-[1450px]:grid-cols-2 max-[1100px]:grid-cols-1">
            ...
            <div class="max-w-sm bg-blue-100 border border-blue-200 rounded-lg shadow hover:shadow-2xl">
                <div class="flex flex-row items-center justify-center ml-6 mt-6 align-baseline shrink-0 grow-0">
                    ...
                    <h5 class="ml-4 text-2xl font-bold text-blue-500 dark:text-white">{{$broker->name}}</h5>
                </div>
                <div class="p-5">
                    <p class="mb-3 font-normal text-gray-400 dark:text-gray-400">{{$broker->description}}</p>
                        <h5 class="mb-2 text-lg font-bold tracking-tight text-blue-500 dark:text-white"> @lang('public.instructor_note'):</h5>
                    <p class="mb-3 font-normal text-gray-700 dark:text-gray-400">{{$broker->note}}</p>
                        <button data-modal-target="brokerModal{{$broker->id}}" data-modal-toggle="brokerModal{{$broker->id}}" class="flex mx-auto items-center px-3 py-2 text-md font-medium text-center text-white bg-blue-500 rounded-lg hover:bg-blue-500 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
            ...
            <div id="brokerModal{{$broker->id}}" tabindex="-1" class="fixed top-0 left-0 right-0 z-50 hidden p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-[calc(100%-1rem)] md:h-full">
                <div class="relative w-full h-full max-w-2xl md:h-auto">
                    <div class="relative bg-blue-100 rounded-lg shadow dark:bg-gray-700 p-2">
                        ...
                        <div class="p-6 space-y-2">
                            <div class="flex items-center space-x-4">
                                ...
                                <div class="font-medium dark:text-white">
                                    <div class="text-xl text-blue-500 font-bold">{{$broker->name}}</div>
                            ...
                            <h5 class="text-lg font-bold tracking-tight text-blue-500 dark:text-white"> @lang('public.instructor_note'):</h5>
